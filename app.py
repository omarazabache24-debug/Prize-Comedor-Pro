...
def login():
    ...
    return render_page("""<div class='login-bg'><div class='login-card'><img src='/static/logo.png?v=final' alt='PRIZE'><h2>Sistema Comedor PRIZE</h2><p class='muted'>Acceso al sistema</p><form method='post'><input name='username' placeholder='Usuario' required><input name='password' type='password' placeholder='Clave' required><button>Ingresar</button></form><p class='mini'>Usuarios de prueba:<br>admin/admin123 · rrhh/rrhh123 · comedor/comedor123</p></div></div>""")
@app.route('/logout')
