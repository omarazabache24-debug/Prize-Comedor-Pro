...
<title>Sistema Comedor PRIZE</title><link rel="icon" href="{{url_for('static',filename='favicon.png')}}"><style>
:root{--green:#159947;--blue:#0b6fad;--orange:#ff6b13;--dark:#061b27;--line:#dfe8ef;--muted:#607089;--text:#082033;--shadow:0 12px 28px rgba(15,23,42,.10)}*{box-sizing:border-box}body{margin:0;font-family:Segoe UI,Arial,sans-serif;background:linear-gradient(180deg,#f8fbfd,#eef7f2);color:var(--text)}a{text-decoration:none;color:inherit}img{max-width:100%;display:block}.hero{margin:10px;border:1px solid var(--line);border-radius:14px;background:#fff;display:grid;grid-template-columns:330px 1fr 310px;gap:24px;align-items:center;padding:18px 24px;box-shadow:var(--shadow)}.hero-logo{height:126px;object-fit:contain;margin:auto}.hero h1{font-size:34px;margin:0 0 7px;font-weight:900}.hero p{font-size:18px;margin:0 0 6px}.checks{font-weight:750;color:#21344a;font-size:15px}.checks div{margin:7px 0}.users{background:#061b27;color:white;border-radius:12px;padding:16px 18px;line-height:1.9;box-shadow:0 10px 20px #001b2a22}.layout{display:grid;grid-template-columns:250px 1fr;gap:20px;min-height:calc(100vh - 185px);padding:0 10px}.side{background:linear-gradient(180deg,#082d43,#061722);color:white;padding:18px;border-radius:16px;box-shadow:var(--shadow);height:max-content;position:sticky;top:10px}.brand{text-align:center;border-bottom:1px solid #ffffff22;padding-bottom:15px;margin-bottom:12px}.brand img{width:112px;height:76px;object-fit:contain;margin:0 auto 8px;background:#fff;border-radius:12px;padding:5px}.brand h3{margin:4px 0}.nav a{display:flex;align-items:center;gap:9px;padding:12px 14px;border-radius:12px;margin:6px 0;color:#dbeafe;font-weight:800}.nav a:hover,.nav .on{background:linear-gradient(90deg,#159947,#0b6fad);color:#fff}.main{min-width:0;padding:4px 0 24px}.top{display:flex;justify-content:space-between;align-items:center;gap:12px;margin:0 0 14px}.top h2{font-size:27px;margin:0;font-weight:900}.muted{color:var(--muted)}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:18px;box-shadow:var(--shadow)}.kpi b{font-size:28px;color:#0b2742}.kpi span{display:block;color:var(--muted);font-size:13px}.form{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}input,select,textarea{width:100%;padding:12px 14px;border:1px solid #cfdae6;border-radius:12px;background:#fff;outline:none}input:focus,select:focus,textarea:focus{border-color:#159947;box-shadow:0 0 0 4px #15994718}button,.btn{border:0;border-radius:12px;padding:12px 16px;background:var(--green);color:white;font-weight:900;cursor:pointer;display:inline-block}.btn2{background:var(--blue)}.btn3{background:var(--orange)}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:14px}table{width:100%;border-collapse:collapse;background:#fff}th,td{padding:11px;border-bottom:1px solid #edf2f7;text-align:left;white-space:nowrap;font-size:13px}th{background:#f7fafc;color:#23364a}.badge{border-radius:999px;padding:5px 10px;font-weight:900;font-size:12px;display:inline-block}.ok{background:#dcfce7;color:#166534}.off{background:#fee2e2;color:#991b1b}.warn{background:#fef3c7;color:#92400e}.flash{margin:10px;padding:12px 14px;border-radius:12px;background:#eff6ff;border:1px solid #bfdbfe}.flash.error{background:#fef2f2;border-color:#fecaca;color:#991b1b}.flash.ok{background:#f0fdf4;border-color:#bbf7d0;color:#166534}.login-shell{min-height:100vh;display:grid;grid-template-columns:minmax(320px,470px) 1fr;gap:22px;align-items:center;padding:28px;background:linear-gradient(135deg,#fff,#f1faf5 55%,#eef7ff)}.login-card{position:relative;overflow:hidden;background:#fff;border:1px solid var(--line);border-radius:18px;padding:38px 42px 30px;box-shadow:var(--shadow);min-height:520px}.login-card:before{content:"";position:absolute;inset:0;background:url('/static/login_referencia.png?v=20260428') center/cover no-repeat;opacity:.10;pointer-events:none}.login-card>*{position:relative}.login-logo{width:250px;height:150px;object-fit:contain;margin:0 auto 10px}.login-card h2{text-align:center;margin:8px 0 4px;font-size:22px}.login-card p{text-align:center}.login-card label{display:block;text-align:left;font-weight:800;margin:16px 0 6px}.login-card button{width:100%;margin-top:18px}.login-wave{position:absolute;left:0;right:0;bottom:0;height:66px;background:linear-gradient(135deg,#063763 0%,#0b6fad 35%,#159947 36%,#159947 100%);clip-path:polygon(0 55%,40% 86%,72% 30%,100% 0,100% 100%,0 100%)}.login-preview{background:#fff;border:1px solid var(--line);border-radius:18px;box-shadow:var(--shadow);padding:18px}.login-preview img{width:100%;border-radius:12px;object-fit:contain}.notice{border-left:5px solid var(--green);padding:11px;background:#f0fdf4;border-radius:12px;margin:10px 0}.footer{background:#061b27;color:#dbeafe;padding:18px 28px;margin-top:10px;display:flex;justify-content:space-between}.mini{font-size:12px;color:#64748b;text-align:center}@media(max-width:1100px){.hero{grid-template-columns:1fr}.layout{grid-template-columns:1fr}.side{position:relative}.grid{grid-template-columns:1fr 1fr}.form{grid-template-columns:1fr}.top{display:block}.login-shell{grid-template-columns:1fr}.login-preview{display:none}}@media(max-width:640px){.grid{grid-template-columns:1fr}.login-shell{padding:10px}.login-card{padding:28px 22px}.hero h1{font-size:26px}}
</style></head><body>
{% if session.get('user') %}<div class="hero"><div><img class="hero-logo" src="{{url_for('static',filename='logo.png')}}?v=20260428" onerror="this.src='{{url_for('static',filename='logo_prize.jpeg')}}'"></div><div><h1>Sistema Comedor PRIZE</h1><p class="muted">ERP para la Gestión del Comedor Corporativo</p><div class="checks"><div>✅ Registro y control de consumos</div><div>✅ Entrega de pedidos validando DNI</div><div>✅ Carga masiva de consumos Excel</div><div>✅ Cierre de día y consolidación de reportes</div><div>✅ Envío automático por correo</div><div>✅ Roles y permisos</div></div></div><div class="users"><b>Usuarios de prueba</b><br>👤 admin / admin123<br>👥 rrhh / rrhh123<br>🍽️ comedor / comedor123</div></div><div class="layout"><aside class="side"><div class="brand"><img src="{{url_for('static',filename='logo.png')}}?v=20260428" onerror="this.src='{{url_for('static',filename='logo_prize.jpeg')}}'"><h3>ERP Comedor</h3><small>{{session.get('user')}} · {{session.get('role')}}</small></div><nav class="nav"><a class="{{'on' if page=='dashboard'}}" href="{{url_for('dashboard')}}">📊 Dashboard</a><a class="{{'on' if page=='pedidos'}}" href="{{url_for('pedidos')}}">🍽️ Consumos</a><a class="{{'on' if page=='entregas'}}" href="{{url_for('entregas')}}">✅ Entregas</a><a class="{{'on' if page=='carga'}}" href="{{url_for('carga_masiva')}}">📥 Carga Masiva</a><a class="{{'on' if page=='trabajadores'}}" href="{{url_for('trabajadores')}}">👥 Trabajadores</a><a class="{{'on' if page=='cierre'}}" href="{{url_for('cierre_dia')}}">🔒 Cierre de Día</a><a href="{{url_for('logout')}}">🚪 Salir</a></nav></aside><main class="main">{% endif %}{% with messages=get_flashed_messages(with_categories=true) %}{% for c,m in messages %}<div class="flash {{c}}">{{m}}</div>{% endfor %}{% endwith %}{{content|safe}}{% if session.get('user') %}</main></div><div class="footer"><span>© 2026 Prize Superfruits - Comedor Corporativo</span><span>Versión 2.0.0</span></div>{% endif %}</body></html>
"""
...
def login():
    ...
    return render_page("""<div class='login-shell'><div class='login-card'><img class='login-logo' src='/static/logo.png?v=20260428' onerror=\"this.src='/static/logo_prize.jpeg'\"><h2>Sistema Comedor PRIZE</h2><p class='muted'>Acceso al sistema</p><form method='post'><label>Usuario</label><input name='username' placeholder='Ingrese su usuario' required><label>Clave</label><input name='password' type='password' placeholder='Ingrese su clave' required><button>Ingresar</button></form><p class='mini'>Usuarios de prueba:<br>admin/admin123 · rrhh/rrhh123 · comedor/comedor123</p><div class='login-wave'></div></div><div class='login-preview'><img src='/static/panel_referencia.png?v=20260428'></div></div>""")
@app.route('/logout')
