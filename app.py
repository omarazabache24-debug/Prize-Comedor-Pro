...
<style>
:root{--green:#159447;--green2:#0d7c36;--navy:#061d2d;--navy2:#082c44;--blue:#0e77bb;--orange:#ff6b13;--purple:#7c3aed;--bg:#f6f9fc;--line:#e6edf4;--muted:#64748b;--text:#102033;--shadow:0 14px 30px rgba(15,23,42,.08)}*{box-sizing:border-box}body{margin:0;font-family:Segoe UI,Inter,Arial,sans-serif;background:linear-gradient(135deg,#f8fbff,#f5fff7);color:var(--text)}a{text-decoration:none;color:inherit}.app-shell{padding:12px}.hero{border:1px solid var(--line);border-radius:18px;background:rgba(255,255,255,.96);display:grid;grid-template-columns:310px 1fr 310px;gap:24px;align-items:center;padding:18px 24px;box-shadow:var(--shadow);margin-bottom:14px}.hero-logo{border-right:1px solid var(--line);height:130px;display:flex;align-items:center}.hero-logo img{max-width:245px;max-height:125px;object-fit:contain}.hero h1{font-size:34px;line-height:1.1;margin:0 0 8px;color:#0b2135}.hero p{font-size:17px;margin:0;color:var(--muted)}.checks{display:grid;grid-template-columns:1fr;gap:7px;font-weight:750;color:#27384c}.check{display:flex;gap:10px;align-items:center}.check:before{content:'✓';display:inline-grid;place-items:center;width:18px;height:18px;border-radius:50%;background:#18a84d;color:#fff;font-size:12px;font-weight:900}.users{background:linear-gradient(135deg,#071c2c,#082c44);color:white;border-radius:14px;padding:18px 20px;line-height:1.75;box-shadow:inset 0 0 0 1px rgba(255,255,255,.08)}.users b{display:block;margin-bottom:6px;font-size:16px}.layout{display:grid;grid-template-columns:176px minmax(0,1fr) 290px;gap:14px;align-items:start}.side{background:linear-gradient(180deg,#082b43,#061722);color:white;border-radius:18px;padding:14px;box-shadow:var(--shadow);position:sticky;top:12px}.brand{text-align:center;border-bottom:1px solid rgba(255,255,255,.14);padding-bottom:12px;margin-bottom:10px}.brand img{width:105px;height:66px;object-fit:contain}.brand h3{margin:4px 0 0;font-size:13px}.brand small{color:#b9d6e6}.nav a{display:flex;align-items:center;gap:9px;padding:11px 10px;border-radius:11px;margin:4px 0;color:#d9edff;font-weight:800;font-size:13px}.nav a:hover,.nav .on{background:linear-gradient(90deg,#12853f,#1079b7);color:white}.main{min-width:0}.top{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:14px}.top h2{font-size:24px;margin:0}.topbar{height:62px;background:#fff;border:1px solid var(--line);border-radius:18px;padding:12px 16px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;box-shadow:var(--shadow)}.topbar .user-pill{display:flex;align-items:center;gap:8px}.avatar{width:36px;height:36px;border-radius:50%;background:#e7f8eb;color:var(--green);display:grid;place-items:center;font-weight:900}.page-title{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:14px}.page-title h2{font-size:24px;margin:0}.muted{color:var(--muted)}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:18px;box-shadow:var(--shadow)}.kpi{display:flex;gap:14px;align-items:center}.ico{width:54px;height:54px;border-radius:50%;display:grid;place-items:center;font-size:26px;background:#eaf8ee}.ico.blue{background:#eaf4ff}.ico.purple{background:#f3e8ff}.ico.orange{background:#fff3e8}.kpi b{font-size:26px;color:#12304a}.kpi span{display:block;color:var(--muted);font-size:13px}.right-panel{display:flex;flex-direction:column;gap:14px}.status-card .big-badge{display:inline-flex;align-items:center;gap:8px;background:#e8f8ed;color:#14813c;border-radius:10px;padding:9px 12px;font-weight:900}.quick a{display:flex;gap:10px;align-items:center;padding:13px;border:1px solid var(--line);border-radius:12px;margin-top:8px;color:#0d4371;font-weight:800;background:#fff}.quick a:hover{border-color:#9ac8ea;background:#f6fbff}.form{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}input,select,textarea{width:100%;padding:12px 13px;border:1px solid #d5e0ea;border-radius:12px;background:white;color:#102033;outline:none}input:focus,select:focus,textarea:focus{border-color:#51a6db;box-shadow:0 0 0 4px rgba(14,119,187,.10)}button,.btn{border:0;border-radius:12px;padding:12px 16px;background:linear-gradient(90deg,#159447,#12a64d);color:white;font-weight:900;cursor:pointer;display:inline-block}.btn2{background:linear-gradient(90deg,#0e77bb,#0b5f9a)}.btn3{background:linear-gradient(90deg,#ff781f,#ff5f0a)}.btn-danger{background:#dc2626}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:14px}table{width:100%;border-collapse:collapse;background:white}th,td{padding:11px 12px;border-bottom:1px solid #edf2f7;text-align:left;white-space:nowrap;font-size:13px}th{background:#f6fafc;color:#30445b;font-weight:900}.badge{border-radius:999px;padding:5px 10px;font-weight:900;font-size:12px;display:inline-block}.ok{background:#dcfce7;color:#166534}.off{background:#fee2e2;color:#991b1b}.warn{background:#fef3c7;color:#92400e}.flash{margin-bottom:12px;padding:12px 14px;border-radius:12px;background:#eff6ff;border:1px solid #bfdbfe}.flash.error{background:#fef2f2;border-color:#fecaca;color:#991b1b}.flash.ok{background:#f0fdf4;border-color:#bbf7d0;color:#166534}.login-wrap{min-height:100vh;display:grid;grid-template-columns:1.08fr .92fr;background:linear-gradient(135deg,#f8fbff,#effff4)}.login-visual{padding:34px;display:flex;flex-direction:column;justify-content:center}.login-panel{background:white;margin:22px;border:1px solid var(--line);border-radius:24px;box-shadow:var(--shadow);display:flex;align-items:center;justify-content:center;padding:32px}.login-card{width:100%;max-width:410px;text-align:center}.login-card img{max-width:260px;max-height:155px;object-fit:contain}.login-card h2{margin:12px 0 4px;font-size:24px}.wave{height:54px;margin:28px -32px -32px;background:linear-gradient(135deg,#074a78,#169b45);border-radius:0 0 24px 24px}.feature-box{background:#fff;border:1px solid var(--line);border-radius:22px;padding:28px;box-shadow:var(--shadow)}.feature-box img{max-width:285px}.feature-box h1{font-size:38px;margin:8px 0;color:#0b2135}.features{margin-top:18px;display:grid;gap:10px}.notice{border-left:5px solid var(--green);padding:11px;background:#f0fdf4;border-radius:12px;margin:10px 0}.filters{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:10px;margin-bottom:12px}.footer{background:#061b27;color:#dbeafe;padding:18px 28px;margin-top:14px;border-radius:0;display:flex;justify-content:space-between}.mini{font-size:12px;color:#64748b}.module-tabs{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 14px}.module-tabs a{padding:10px 13px;border:1px solid var(--line);border-radius:999px;background:#fff;color:#0b3150;font-weight:800}.module-tabs a.active{background:#0b6fad;color:#fff;border-color:#0b6fad}@media(max-width:1180px){.hero,.layout,.login-wrap{grid-template-columns:1fr}.right-panel{display:grid;grid-template-columns:1fr 1fr}.side{position:relative;top:0}.nav{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.hero-logo{border-right:0;height:auto}.login-visual{padding:20px}.login-panel{margin:10px}.grid{grid-template-columns:1fr 1fr}}@media(max-width:720px){.grid,.form,.filters,.right-panel{grid-template-columns:1fr}.hero h1{font-size:27px}.app-shell{padding:8px}.nav{grid-template-columns:1fr 1fr}.page-title,.topbar{display:block;height:auto}.footer{display:block}.login-wrap{display:block}.login-visual{display:none}}
</style></head><body>
{% if session.get('user') %}<div class="app-shell"><div class="hero"><div class="hero-logo"><img src="{{url_for('static',filename='logo.png')}}?v=12"></div><div><h1>Sistema Comedor PRIZE</h1><p>ERP para la Gestión del Comedor Corporativo</p></div><div class="users"><b>Usuarios de prueba</b>🟢 admin / admin123<br>🟣 rrhh / rrhh123<br>🟠 comedor / comedor123</div></div><div class="layout"><aside class="side"><div class="brand"><img src="{{url_for('static',filename='logo.png')}}?v=12"><h3>ERP Comedor</h3><small>{{session.get('user')}} · {{session.get('role')}}</small></div><nav class="nav"><a class="{{'on' if page=='dashboard'}}" href="{{url_for('dashboard')}}">📊 Dashboard</a><a class="{{'on' if page=='pedidos'}}" href="{{url_for('pedidos')}}">🍽️ Consumos</a><a class="{{'on' if page=='entregas'}}" href="{{url_for('entregas')}}">✅ Entregas</a><a class="{{'on' if page=='trabajadores'}}" href="{{url_for('trabajadores')}}">👥 Trabajadores</a><a class="{{'on' if page=='carga'}}" href="{{url_for('carga_masiva')}}">📥 Carga Masiva</a><a class="{{'on' if page=='cierre'}}" href="{{url_for('cierre_dia')}}">🔒 Cierre de Día</a><a href="{{url_for('logout')}}">🚪 Salir</a></nav></aside><main class="main"><div class="topbar"><div><b>{{ {'dashboard':'Dashboard','pedidos':'Consumos','entregas':'Entrega de Pedidos','trabajadores':'Trabajadores','carga':'Carga Masiva','cierre':'Cierre de Día'}.get(page,'Sistema') }}</b><br><span class="muted">Sistema Comedor PRIZE</span></div><div class="user-pill">🔔 <div class="avatar">{{session.get('user','U')[0]|upper}}</div><div><small class="muted">Bienvenido,</small><br><b>{{session.get('user')}}</b></div></div></div><div class="module-tabs"><a class="{{'active' if page=='dashboard'}}" href="{{url_for('dashboard')}}">Dashboard</a><a class="{{'active' if page=='pedidos'}}" href="{{url_for('pedidos')}}">Registrar consumo</a><a class="{{'active' if page=='entregas'}}" href="{{url_for('entregas')}}">Entregar pedido</a><a class="{{'active' if page=='carga'}}" href="{{url_for('carga_masiva')}}">Carga Excel</a><a class="{{'active' if page=='cierre'}}" href="{{url_for('cierre_dia')}}">Enviar reporte</a></div>{% endif %}{% with messages=get_flashed_messages(with_categories=true) %}{% for c,m in messages %}<div class="flash {{c}}">{{m}}</div>{% endfor %}{% endwith %}{{content|safe}}{% if session.get('user') %}</main><aside class="right-panel"><div class="card status-card"><h3>Estado del día</h3><div class="big-badge">📗 Día abierto</div><p class="muted">Fecha actual del sistema</p><a class="btn btn3" href="{{url_for('cierre_dia')}}" style="width:100%;text-align:center">Cerrar día y consolidar</a></div><div class="card quick"><h3>Acciones rápidas</h3><a href="{{url_for('pedidos')}}">🧾 Registrar consumo</a><a href="{{url_for('entregas')}}">🚚 Entrega de pedidos</a><a href="{{url_for('carga_masiva')}}">📥 Carga masiva de consumos</a><a href="{{url_for('cierre_dia')}}">✉️ Enviar reporte por correo</a></div></aside></div><div class="footer"><span>© 2026 Prize Superfruits - Comedor Corporativo</span><span>Versión 2.0.0</span></div></div>{% endif %}</body></html>
"""
